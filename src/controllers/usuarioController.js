...
export const criarUsuario = async (req, res) => {
  try {
    ...
    // Validar role
    const roleValida = ["ADMIN", "FUNCIONARIO", "FINANCEIRO"].includes(role);
    if (!roleValida) {
      return res
        .status(400)
        .json({ error: "Role inválida. Use ADMIN, FUNCIONARIO ou FINANCEIRO" });
    }
...
export const atualizarUsuario = async (req, res) => {
  try {
    ...
    if (role && !["ADMIN", "FUNCIONARIO", "FINANCEIRO"].includes(role)) {
      return res
        .status(400)
        .json({ error: "Role inválida. Use ADMIN, FUNCIONARIO ou FINANCEIRO" });
    }
...
// Listar apenas funcionários ativos (para dropdown de atribuição de roteiros)
